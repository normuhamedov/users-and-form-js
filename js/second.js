...
const allDatas = users.map((element) => {
    return `
    <div class="col-lg-4">
    <div style="background-color: rgb(12, 0, 44);" class="card" style="width: 100%;">
        <img src=${element.avatar_url}
            class="card-img rounded-circle" alt=${element.login}>
        <div class="card-body text-primary">
            <h5 class="card-title">${element.login}</h5>
            <p class="card-text">${element.id}</p>
             <a class="btn btn-primary" href="${element.html_url}">Git Hub</a>
        </div>
    </div>
</div>
    `
